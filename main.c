...
static int update_loop(void *data)
{
    ...
    if (g_ctrl.up)
        try_move(&g_player_pos, g_player_pos.x + dir.x * move_speed, g_player_pos.y + dir.y * move_speed);
    if (g_ctrl.down)
        try_move(&g_player_pos, g_player_pos.x - dir.x * move_speed, g_player_pos.y - dir.y * move_speed);
    if (g_ctrl.right)
        try_move(&g_player_pos, g_player_pos.x + right.x * move_speed, g_player_pos.y + right.y * move_speed);
    if (g_ctrl.left)
        try_move(&g_player_pos, g_player_pos.x - right.x * move_speed, g_player_pos.y - right.y * move_speed);
    ...
    /* cast a single ray from player in direction */
    double hx, hy;
    cast_ray(g_player_pos, dir, 20.0, &hx, &hy);

    int sx = (int)(g_player_pos.x * TILE_SIZE);
    int sy = (int)(g_player_pos.y * TILE_SIZE);
    int ex = (int)(hx * TILE_SIZE);
    int ey = (int)(hy * TILE_SIZE);
    draw_line(frame, sx, sy, ex, ey, 0x00FF00);
